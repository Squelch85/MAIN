...
def load_state(state_path):
    ...
    반환 값은 ``(geometry, files, file_states, zoom)`` 튜플이며,
    ``file_states``에는 각 파일별 UI 정보가 저장됩니다.
    상태 파일이 없거나 해당 키가 없으면 ``file_states`` 는 빈 딕셔너리를,
    ``zoom`` 값이 없으면 ``1.0`` 을 돌려줍니다.
    """
    saved_geometry = None
    open_files = []
    file_states = {}
    ...
    return saved_geometry, open_files, file_states, zoom


def save_state(state_path, geometry, files, file_states, zoom=1.0):
